-- Ładowanie danych KSEF na podstawie tabeli uprawnień
-- Role: Księgowa, Handlowiec, Zakupowiec, Administrator, Właściciel (K+A)

-- 1. Dodaj aplikację KSEF (bez updated_at)
...
-- 2. Dodaj role KSEF (według tabeli, bez polskich znaków)
...
-- 4. Utwórz profile aplikacyjne (bez polskich znaków)
...
-- 5. Mapowanie ról na uprawnienia (na podstawie tabeli z X-ami, bez polskich znaków)
...
SELECT 'UPRAWNIENIA KSEF:' as info;
SELECT permission_name FROM permissions WHERE app_id = 'ksef' ORDER BY permission_name;
